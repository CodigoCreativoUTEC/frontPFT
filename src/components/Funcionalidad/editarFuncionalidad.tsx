...
import {FuncionalidadModel, ReferrerEnum, UsuarioModel} from "@/types";
...
const EditFuncionalidad = () => {
  ...
  const handleChange = (e: { target: { name: any; value: any; }; }) => {
    ...
    // @ts-ignore
    setFuncionalidad({estado: undefined, id: undefined, nombre: "", ...funcionalidad, [name]: value });
  };

  const validateForm = () => {
    const newErrors = [];
    // @ts-ignore
    if (!funcionalidad.nombre) newErrors.push("El nombre de la funcionalidad es obligatorio");
    // @ts-ignore
    setErrors(newErrors);
...
const EditFuncionalidad = () => {
    const router = useRouter();
    const { id } = useParams();
    const [funcionalidad, setFuncionalidad] = useState({ id: null, nombre: '', estado: '' });
    const [errors, setErrors] = useState([]);

    useEffect(() => {
        const foundFuncionalidad = initialFuncionalidades.find(f => f.id === Number(id));
        // @ts-ignore
        setFuncionalidad(foundFuncionalidad || { id: null, nombre: '', estado: '' });
    }, [id]);

    const handleChange = (e: { target: { name: any; value: any; }; }) => {
        const { name, value } = e.target;
        setFuncionalidad({ ...funcionalidad, [name]: value });
    };

    const validateForm = () => {
        const newErrors = [];
        if (!funcionalidad.nombre) newErrors.push("El nombre es obligatorio");
        // @ts-ignore
        setErrors(newErrors);
        return newErrors.length === 0;
    };
    ...
    const handleSubmit = () => {
        ...
        router.push('/funcionalidades'); // Redirigir al listado de funcionalidades
    };
