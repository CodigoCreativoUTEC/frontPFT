...
import { IntervencionModel } from "@/types"; // Asegúrate de que el modelo esté definido correctamente en tus types

interface IntervencionesListProps extends IntervencionModel {
    fetcher: () => void;
}

const IntervencionesList: React.FC<IntervencionesListProps> = (params) => {
    const { data: session, status } = useSession();
    const [showConfirmModal, setShowConfirmModal] = useState<boolean>(false);
    const [intervencionIdToDelete, setIntervencionIdToDelete] = useState<number | null>(null);

    const handleDeleteClick = (id: number | null) => {
        setIntervencionIdToDelete(id);
        setShowConfirmModal(true);
    };

    const borrarIntervencion = async () => {
        if (intervencionIdToDelete === null) return;

        try {
            const res = await fetch(`http://localhost:8080/ServidorApp-1.0-SNAPSHOT/api/intervenciones/eliminar?id=${intervencionIdToDelete}`, {
                method: 'DELETE',
                headers: {
            ...
            if (!res.ok) {
                ...
                throw new Error(errorData.error || 'Error al eliminar la intervención');
            }
            ...
            setIntervencionIdToDelete(null);
        }
    };
            ...
            {showConfirmModal && (
                <div className='fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center'>
                    <div className='rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark p-5'>
                        ...
                        <p>¿Estás seguro de que deseas eliminar esta intervención?</p>
                        <div className='mt-4'>
                            <button
