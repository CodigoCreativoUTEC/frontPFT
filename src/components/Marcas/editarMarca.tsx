...
const EditMarca = () => {
  ...
  const handleSubmit = async () => {
    if (validateForm()) {
      const res = await fetch(`http://localhost:8080/ServidorApp-1.0-SNAPSHOT/api/marcas/modificar`, {
        method: 'PUT',
        headers: {
          "Content-Type": "application/json",
          "authorization": "Bearer " + (session?.user?.accessToken || ''),
...
const EditMarca = () => {
  const router = useRouter();
  const { id } = useParams();
  const [marca, setMarca] = useState({ id: null, nombre: '', estado: '' });
  const [errors, setErrors] = useState([]);

  useEffect(() => {
    const foundMarca = initialMarcas.find(m => m.id === Number(id));
    // @ts-ignore
    setMarca(foundMarca || { id: null, nombre: '', estado: '' });
  }, [id]);
  ...
  const validateForm = () => {
    const newErrors = [];
    if (!marca.nombre) newErrors.push("El nombre es obligatorio");
    // @ts-ignore
    setErrors(newErrors);
    return newErrors.length === 0;
  };
  ...
  const handleSubmit = () => {
    ...
    router.push('/marcas'); // Redirigir al listado de marcas
  };
