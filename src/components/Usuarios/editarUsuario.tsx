...
const EditUsuario = () => {
  ...
  useEffect(() => {
    if (id) {
      const fetchUsuario = async () => {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/usuarios/seleccionar?id=${id}`, {
          headers: {
  ...
  }, [id, session.accessToken, session.user.accessToken]);
  ...
  const handleSubmit = async () => {
    if (usuario && validateForm()) {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/usuarios/modificar`, {
        method: 'PUT',
        headers: {
          "Content-Type": "application/json",
          "authorization": "Bearer " + (session.accessToken || ''),
