...
const LoginForm = () => {
    ...
    const [usarLdap, setUsarLdap] = useState(true);
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const result = await signIn("credentials", {
            ...
            email,
            password,
            usarLdap: usarLdap.toString(), // este es el flag
        });
        ...
        <form onSubmit={handleSubmit}>
            ...
            <div className="mb-4 flex items-center space-x-2">
                <input
                    ...
                    type="checkbox"
                    checked={usarLdap}
                    onChange={() => setUsarLdap(!usarLdap)}
                    className="h-4 w-4 text-primary focus:ring-2 focus:ring-primary border-gray-300 rounded"
                />
                <label htmlFor="usarLdap" className="text-sm text-black dark:text-white">
                    Validar contra Active Directory (AD)
                </label>
            </div>
