...
const EditPerfil = () => {
  ...
  const handleChange = (e: { target: { name: any; value: any; }; }) => {
    ...
    // @ts-ignore
    setPerfil({estado: undefined, id: undefined, nombre: "", ...perfil, [name]: value });
  };

  const validateForm = () => {
    const newErrors = [];
    // @ts-ignore
    if (!perfil.nombre) newErrors.push("El nombre del perfil es obligatorio");
    // @ts-ignore
    setErrors(newErrors);
...
const EditPerfil = () => {
    const router = useRouter();
    const { id } = useParams();
    const [perfil, setPerfil] = useState({ id: null, nombre: '', estado: '' });
    const [errors, setErrors] = useState([]);

    useEffect(() => {
        const foundPerfil = initialPerfiles.find(p => p.id === Number(id));
        // @ts-ignore
        setPerfil(foundPerfil || { id: null, nombre: '', estado: '' });
    }, [id]);
    ...
    const validateForm = () => {
        const newErrors = [];
        if (!perfil.nombre) newErrors.push("El nombre es obligatorio");
        // @ts-ignore
        setErrors(newErrors);
        return newErrors.length === 0;
    };
    ...
    const handleSubmit = () => {
        ...
        router.push('/perfiles'); // Redirigir al listado de perfiles
    };
